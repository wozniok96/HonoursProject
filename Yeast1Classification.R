...
df <- read.csv("yeast1.dat", header= FALSE, stringsAsFactors = FALSE,
               skip = 13)
colnames(df)

#Change class value into integers
df$V9[df$V9 == " positive"] <- 0
df$V9[df$V9 == " negative"] <- 1
df$V9<- as.factor(df$V9)
#Produce a Class Distribution graph
classDist<-table(df$V9)
barplot(classDist, main="Class Distribution", 
        xlab="V9")

#Split the dataset into training and testing set
train.index <- createDataPartition(df$V9, p = .7, list = FALSE)
...
trainStd$V3<-(train$V3-mean(train$V3))/sd(train$V3)
...
testStd$V3<-(test$V3-mean(train$V3))/sd(train$V3)
...
ggplot(trainStd, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)
...
bestSVM<- train(V9~., data = trainStd, method = "svmRadial", metric="Kappa",scale = FALSE,
                trControl=trctrl,
                tuneGrid = tuneValues)
...
bestRF <-train(V9~., data = trainStd, method = "rf", metric="Kappa",scale = FALSE,
               ntree=500,
               trControl=trctrl,
...
weighedBestSVM<- train(V9~., data = trainStd, method = "svmRadialWeights",  metric="Kappa",scale = FALSE,
                       trControl=trctrl,
...
#Test the best model on testing data
weighedPrediction <- predict(weighedBestSVM, testStd[1:8])
summary(weighedPrediction)
weighedTab <- table(pred=weighedPrediction, true=testStd[,9])
...
weighedF1Vec<-c(weighedF1Vec, F1_Score(testStd[,9], weighedPrediction))
...
overTrain <- ovun.sample(V9~., trainStd, method="over", p=.5)$data


overClassDist<-table(overTrain$V9)
barplot(overClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(overTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestOverSVM<- train(V9~., data = overTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
overPrediction <- predict(bestOverSVM, testStd[1:8])
overPrediction<- relevel(overPrediction, "0")
#Create a confusion matrix for the best model.
overTab <- table(pred=overPrediction, true=testStd[,9])
...
overF1Vec<-c(overF1Vec, F1_Score(testStd[,9], overPrediction))
...
bestOverRF <-train(V9~., data = overTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
overPredictionRF <- predict(bestOverRF, testStd[1:8])
overPredictionRF<- relevel(overPredictionRF, "0")
summary(overPredictionRF)

#Create a confusion matrix for the best model.
overTabRF <- table(pred=overPredictionRF, true=testStd[,9])
...
overF1VecRF<-c(overF1VecRF, F1_Score(testStd[,9], overPredictionRF))
...
underTrain <- ovun.sample(V9~., trainStd, method="under", p=.5)$data


underClassDist<-table(underTrain$V9)
barplot(underClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(underTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestUnderSVM <- train(V9~., data = underTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
...
#Test the best model on testing data
underPrediction <- predict(bestUnderSVM, testStd[1:8])
underPrediction<- relevel(underPrediction, "0")
summary(underPrediction)

#Create a confusion matrix for the best model.
underTab <- table(pred=underPrediction, true=testStd[,9])
...
underF1Vec<-c(underF1Vec, F1_Score(testStd[,9], underPrediction))
...
bestUnderRF <-train(V9~., data = underTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
underPredictionRF <- predict(bestUnderRF, testStd[1:8])
underPredictionRF<- relevel(underPredictionRF, "0")
summary(underPredictionRF)

#Create a confusion matrix for the best model.
underTabRF <- table(pred=underPredictionRF, true=testStd[,9])
...
underF1VecRF<-c(underF1VecRF, F1_Score(testStd[,9], underPredictionRF))
...
#Create new training set using undersampling.
bothTrain <- ovun.sample(V9~., trainStd, method="both", p=.5)$data


bothClassDist<-table(bothTrain$V9)
barplot(bothClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(bothTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBothSVM <-train(V9~., data = bothTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
bothPrediction <- predict(bestBothSVM, testStd[1:8])
bothPrediction<- relevel(bothPrediction, "0")
summary(bothPrediction)

#Create a confusion matrix for the best model.
bothTab <- table(pred=bothPrediction, true=testStd[,9])
...
bothF1Vec<-c(bothF1Vec, F1_Score(testStd[,9], bothPrediction))
...
bestBothRF <-train(V9~., data = bothTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
bothPredictionRF <- predict(bestBothRF, testStd[1:8])
bothPredictionRF<- relevel(bothPredictionRF, "0")
summary(bothPredictionRF)

#Create a confusion matrix for the best model.
bothTabRF <- table(pred=bothPredictionRF, true=testStd[,9])
...
bothF1VecRF<-c(bothF1VecRF, F1_Score(testStd[,9], bothPredictionRF))
...
roseTrain <- ROSE(V9~., trainStd, p=0.5)$data


roseClassDist<-table(roseTrain$V9)
barplot(roseClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(roseTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestRoseSVM<- train(V9~., data = roseTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
rosePrediction <- predict(bestRoseSVM, testStd[1:8])
rosePrediction<- relevel(rosePrediction, "0")
summary(rosePrediction)

#Create a confusion matrix for the best model.
roseTab <- table(pred=rosePrediction, true=testStd[,9])
...
roseF1Vec<-c(roseF1Vec, F1_Score(testStd[,9], rosePrediction))
...
bestRoseRF <-train(V9~., data = roseTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
rosePredictionRF <- predict(bestRoseRF, testStd[1:8])
rosePredictionRF<- relevel(rosePredictionRF, "0")
summary(rosePredictionRF)

#Create a confusion matrix for the best model.
roseTabRF <- table(pred=rosePredictionRF, true=testStd[,9])
...
roseF1VecRF<-c(roseF1VecRF, F1_Score(testStd[,9], rosePredictionRF))
...
smoteTrain<-SMOTE(trainStd[,-9], trainStd[,9], K = 3, dup_size = 0)$data
colnames(smoteTrain)[9] <- "V9"
smoteTrain$V9 <- as.factor(smoteTrain$V9)

smoteClassDist<-table(smoteTrain$V9)
barplot(smoteClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(smoteTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSmoteSVM <- train(V9~., data =smoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
bestSmoteSVM
plot(bestSmoteSVM)
...
#Test the best model on testing data
smotePrediction <- predict(bestSmoteSVM, testStd[1:8])
smotePrediction<- relevel(smotePrediction, "0")
summary(smotePrediction)

#Create a confusion matrix for the best model.
smoteTab <- table(pred=smotePrediction, true=testStd[,9])
...
smoteF1Vec<-c(smoteF1Vec, F1_Score(testStd[,9], smotePrediction))
...
bestSmoteRF <-train(V9~., data = smoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
smotePredictionRF <- predict(bestSmoteRF, testStd[1:8])
smotePredictionRF<- relevel(smotePredictionRF, "0")
summary(smotePredictionRF)

#Create a confusion matrix for the best model.
smoteTabRF <- table(pred=smotePredictionRF, true=testStd[,9])
...
smoteF1VecRF<-c(smoteF1VecRF, F1_Score(testStd[,9], smotePredictionRF))
...
blsTrain<-BLSMOTE(trainStd[,-9], trainStd[,9],K=3,C=3,dupSize=0,method = "type1")$data
colnames(blsTrain)[9] <- "V9"
blsTrain$V9 <- as.factor(blsTrain$V9)

blsClassDist<-table(blsTrain$V9)
barplot(blsClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(blsTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBlsSVM <- train(V9~., data = blsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
blsPrediction <- predict(bestBlsSVM, testStd[1:8])
blsPrediction<- relevel(blsPrediction, "0")
summary(blsPrediction)

#Create a confusion matrix for the best model.
blsTab <- table(pred=blsPrediction, true=testStd[,9])
...
blsF1Vec<-c(blsF1Vec, F1_Score(testStd[,9], blsPrediction))
...
bestBlsRF <-train(V9~., data = blsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
blsPredictionRF <- predict(bestBlsRF, testStd[1:8])
blsPredictionRF<- relevel(blsPredictionRF, "0")
summary(blsPredictionRF)

#Create a confusion matrix for the best model.
blsTabRF <- table(pred=blsPredictionRF, true=testStd[,9])
...
blsF1VecRF<-c(blsF1VecRF, F1_Score(testStd[,9], blsPredictionRF))
...
slsTrain<-SLS(trainStd[,-9], trainStd[,9], K = 3, dupSize = 0)$data
colnames(slsTrain)[9] <- "V9"
slsTrain$V9 <- as.factor(slsTrain$V9)

slsClassDist<-table(slsTrain$V9)
barplot(slsClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(slsTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSlsSVM<- train(V9~., data = slsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestSlsSVM
plot(bestSlsSVM)
...
#Test the best model on testing data
slsPrediction <- predict(bestSlsSVM, testStd[1:8])
slsPrediction<- relevel(slsPrediction, "0")
summary(slsPrediction)

#Create a confusion matrix for the best model.
slsTab <- table(pred=slsPrediction, true=testStd[,9])
...
slsF1Vec<-c(slsF1Vec, F1_Score(testStd[,9], slsPrediction))
...
bestSlsRF <-train(V9~., data = slsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
slsPredictionRF <- predict(bestSlsRF, testStd[1:8])
slsPredictionRF<- relevel(slsPredictionRF, "0")
summary(slsPredictionRF)

#Create a confusion matrix for the best model.
slsTabRF <- table(pred=slsPredictionRF, true=testStd[,9])
...
slsF1VecRF<-c(slsF1VecRF, F1_Score(testStd[,9], slsPredictionRF))
...
adasynTrain<-ADAS(trainStd[,-9], trainStd[,9], K = 3)$data
colnames(adasynTrain)[9] <- "V9"
adasynTrain$V9 <- as.factor(adasynTrain$V9)

adasynClassDist<-table(adasynTrain$V9)
barplot(adasynClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(adasynTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestAdasynSVM <- train(V9~., data = adasynTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                       trControl=trctrl,
                       tuneGrid = tuneValues)
bestAdasynSVM
plot(bestAdasynSVM)
...
#Test the best model on testing data
adasynPrediction <- predict(bestAdasynSVM, testStd[1:8])
adasynPrediction<- relevel(adasynPrediction, "0")
summary(adasynPrediction)

#Create a confusion matrix for the best model.
adasynTab <- table(pred=adasynPrediction, true=testStd[,9])
...
adasynF1Vec<-c(adasynF1Vec, F1_Score(testStd[,9], adasynPrediction))
...
bestAdasynRF <-train(V9~., data = adasynTrain, method = "rf", metric="Kappa",scale = FALSE,
                     ntree=500,
                     trControl=trctrl,
                     tuneGrid = tuneRfValues)
...
adasynPredictionRF <- predict(bestAdasynRF, testStd[1:8])
adasynPredictionRF<- relevel(adasynPredictionRF, "0")
summary(adasynPredictionRF)

#Create a confusion matrix for the best model.
adasynTabRF <- table(pred=adasynPredictionRF, true=testStd[,9])
...
adasynF1VecRF<-c(adasynF1VecRF, F1_Score(testStd[,9], adasynPredictionRF))
...
dbsTrain<-DBSMOTE(trainStd[,-9], trainStd[,9] )$data
colnames(dbsTrain)[9] <- "V9"
dbsTrain$V9 <- as.factor(dbsTrain$V9)

dbsClassDist<-table(dbsTrain$V9)
barplot(dbsClassDist, main="Class Distribution", 
        xlab="V9")

ggplot(dbsTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestDbsSVM<- train(V9~., data = dbsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestDbsSVM
plot(bestDbsSVM)
...
#Test the best model on testing data
dbsPrediction <- predict(bestDbsSVM, testStd[1:8])
summary(dbsPrediction)

#Create a confusion matrix for the best model.
dbsTab <- table(pred=dbsPrediction, true=testStd[,9])
...
dbsF1Vec<-c(dbsF1Vec, F1_Score(testStd[,9], dbsPrediction))
...
bestDbsRF <-train(V9~., data = dbsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
dbsPredictionRF <- predict(bestDbsRF, testStd[1:8])
dbsPrediction<- relevel(dbsPrediction, "0")
summary(dbsPredictionRF)

#Create a confusion matrix for the best model.
dbsTabRF <- table(pred=dbsPredictionRF, true=testStd[,9])
...
dbsF1VecRF<-c(dbsF1VecRF, F1_Score(testStd[,9], dbsPredictionRF))
...
#Create a new training set using SMOTE method
tlsmoteTrain<-SMOTE(trainStd[,-9], trainStd[,9], K = 3, dup_size = 0)$data
colnames(tlsmoteTrain)[9] <- "V9"
tlsmoteTrain$V9 <- as.factor(smoteTrain$V9)
#Apply Tomek Links removal
TLsmoteTrain <- TomekClassif(V9~., tlsmoteTrain, dist = "Euclidean", 
                             Cl = "all", rem = "both")[[1]]

tlsmoteClassDist<-table(TLsmoteTrain$V9)
barplot(tlsmoteClassDist, main="Class Distribution", 
        xlab="Survival Status")

ggplot(TLsmoteTrain, aes(x=V2, y=V3, color=V9)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
besttlSmoteSVM <- train(V9~., data =TLsmoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                        trControl=trctrl,
                        tuneGrid = tuneValues)
besttlSmoteSVM
plot(besttlSmoteSVM)
...
#Test the best model on testing data
tlsmotePrediction <- predict(besttlSmoteSVM, testStd[1:8])
summary(tlsmotePrediction)

#Create a confusion matrix for the best model.
tlsmoteTab <- table(pred=tlsmotePrediction, true=testStd[,9])
...
tlsmoteF1Vec<-c(tlsmoteF1Vec, F1_Score(testStd[,9], tlsmotePrediction))
...
besttlSmoteRF <-train(V9~., data = TLsmoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                      ntree=500,
                      trControl=trctrl,
                      tuneGrid = tuneRfValues)
...
tlsmotePredictionRF <- predict(besttlSmoteRF, testStd[1:8])
summary(tlsmotePredictionRF)

#Create a confusion matrix for the best model.
tlsmoteTabRF <- table(pred=tlsmotePredictionRF, true=testStd[,9])
...
tlsmoteF1VecRF<-c(tlsmoteF1VecRF, F1_Score(testStd[,9], tlsmotePredictionRF))
...
SensGraph + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(Sensitivity, digits=3)), vjust=1.6, color="black",
                                                                             position = position_dodge(0.9), size=4)
###########################################################################################################
SpecGraph <- ggplot(resultsDf, aes(x=MethodName, y=Specificity,fill=Specificity))

SpecGraph + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(Specificity, digits=3)), vjust=1.6, color="black",
                                                                             position = position_dodge(0.9), size=4)
###########################################################################################################
F1Graph <- ggplot(resultsDf, aes(x=MethodName, y=F1_Score, fill=F1_Score))

F1Graph + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(F1_Score, digits=3)), vjust=1.6, color="black",
                                                                           position = position_dodge(0.9), size=4)
##########################################################################################################
GmeanGraph <- ggplot(resultsDf, aes(x=MethodName, y=G_Mean, fill=G_Mean))

GmeanGraph + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(G_Mean, digits=3)), vjust=1.6, color="black",
                                                                              position = position_dodge(0.9), size=4)
###########################################################################################################
###########################################################################################################
###########################################################################################################
...
SensGraphRF + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(Sensitivity, digits=3)), vjust=1.6, color="black",
                                                                               position = position_dodge(0.9), size=4)
###########################################################################################################
SpecGraphRF <- ggplot(RFresultsDf, aes(x=MethodName, y=Specificity, fill=Specificity))

SpecGraphRF + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(Specificity, digits=3)), vjust=1.6, color="black",
                                                                               position = position_dodge(0.9), size=4)
###########################################################################################################
F1GraphRF <- ggplot(RFresultsDf, aes(x=MethodName, y=F1_Score, fill=F1_Score))

F1GraphRF + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(F1_Score, digits=3)), vjust=1.6, color="black",
                                                                             position = position_dodge(0.9), size=4)
##########################################################################################################
GmeanGraphRF <- ggplot(RFresultsDf, aes(x=MethodName, y=G_Mean, fill=G_Mean))

GmeanGraphRF + geom_bar(stat = "identity", position=position_dodge())+geom_text(aes(label=round(G_Mean, digits=3)), vjust=1.6, color="black",
                                                                                position = position_dodge(0.9), size=4)
...
save.image(file='Yeast1MyEnvironment.RData')
dir()
load('Yeast1MyEnvironment.RData')
