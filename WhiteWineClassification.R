#White Wine Dataset 8-9 vs Rest IR =26.21
...
df <- read.csv("winequality-white.csv", header= FALSE, stringsAsFactors = FALSE,sep=";", skip=1)
colnames(df)
...
classDist<-table(df$V12)
barplot(classDist, main="Class Distribution", 
        xlab="V12")

count0<-length(which(df$V12 == 0))
count1<-length(which(df$V12 == 1))
count0
count1
...
ggplot(trainStd, aes(x=V9, y=V11, color=V12)) + geom_point(shape=16)
...
tuneRfValues <- expand.grid(.mtry=c(2,3))
##########################################################################################################################
##################################################################################################################
# sensVec <- vector()
# specVec <- vector()
...
bestSVM<- train(V12~., data = trainStd, method = "svmRadial", metric="Kappa", scale = FALSE,
                trControl=trctrl,
                tuneGrid = tuneValues)
...
bestRF <-train(V12~., data = trainStd, method = "rf", metric="Kappa",scale = FALSE,
               ntree=500,
               trControl=trctrl,
...
PredictionRF <- predict(bestRF, testStd[1:11])
summary(PredictionRF)

#Create a confusion matrix for the best model.
TabRF <- table(pred=PredictionRF, true=testStd[,12])
ResultsRF <- confusionMatrix(TabRF)
ResultsRF

sensVecRF<-c(sensVecRF, ResultsRF$byClass[[1]])
specVecRF<-c(specVecRF, ResultsRF$byClass[[2]])
F1VecRF<-c(F1VecRF, F1_Score(testStd[,12], PredictionRF))
...
weighedBestSVM<- train(V12~., data = trainStd, method = "svmRadialWeights",  metric="Kappa",scale = FALSE,
                       trControl=trctrl,
...
#Test the best model on testing data
weighedPrediction <- predict(weighedBestSVM, testStd[1:11])
summary(weighedPrediction)
weighedTab <- table(pred=weighedPrediction, true=testStd[,12])
...
weighedF1Vec<-c(weighedF1Vec, F1_Score(testStd[,12], weighedPrediction))
...
overTrain <- ovun.sample(V12~., trainStd, method="over", p=.5)$data


overClassDist<-table(overTrain$V12)
barplot(overClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(overTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestOverSVM<- train(V12~., data = overTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
overPrediction <- predict(bestOverSVM, testStd[1:11])
overPrediction<- relevel(overPrediction, "0")
#Create a confusion matrix for the best model.
overTab <- table(pred=overPrediction, true=testStd[,12])
...
overF1Vec<-c(overF1Vec, F1_Score(testStd[,12], overPrediction))
...
bestOverRF <-train(V12~., data = overTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
overPredictionRF <- predict(bestOverRF, testStd[1:11])
overPredictionRF<- relevel(overPredictionRF, "0")
summary(overPredictionRF)

#Create a confusion matrix for the best model.
overTabRF <- table(pred=overPredictionRF, true=testStd[,12])
...
overF1VecRF<-c(overF1VecRF, F1_Score(testStd[,12], overPredictionRF))
...
underTrain <- ovun.sample(V12~., trainStd, method="under", p=.5)$data


underClassDist<-table(underTrain$V12)
barplot(underClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(underTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestUnderSVM <- train(V12~., data = underTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
...
#Test the best model on testing data
underPrediction <- predict(bestUnderSVM, testStd[1:11])
underPrediction<- relevel(underPrediction, "0")
summary(underPrediction)

#Create a confusion matrix for the best model.
underTab <- table(pred=underPrediction, true=testStd[,12])
...
underF1Vec<-c(underF1Vec, F1_Score(testStd[,12], underPrediction))
...
bestUnderRF <-train(V12~., data = underTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
underPredictionRF <- predict(bestUnderRF, testStd[1:11])
underPredictionRF<- relevel(underPredictionRF, "0")
summary(underPredictionRF)

#Create a confusion matrix for the best model.
underTabRF <- table(pred=underPredictionRF, true=testStd[,12])
...
underF1VecRF<-c(underF1VecRF, F1_Score(testStd[,12], underPredictionRF))
...
#Create new training set using undersampling.
bothTrain <- ovun.sample(V12~., trainStd, method="both", p=.5)$data


bothClassDist<-table(bothTrain$V12)
barplot(bothClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(bothTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBothSVM <-train(V12~., data = bothTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
bothPrediction <- predict(bestBothSVM, testStd[1:11])
bothPrediction<- relevel(bothPrediction, "0")
summary(bothPrediction)

#Create a confusion matrix for the best model.
bothTab <- table(pred=bothPrediction, true=testStd[,12])
...
bothF1Vec<-c(bothF1Vec, F1_Score(testStd[,12], bothPrediction))
...
bestBothRF <-train(V12~., data = bothTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
bothPredictionRF <- predict(bestBothRF, testStd[1:11])
bothPredictionRF<- relevel(bothPredictionRF, "0")
summary(bothPredictionRF)

#Create a confusion matrix for the best model.
bothTabRF <- table(pred=bothPredictionRF, true=testStd[,12])
...
bothF1VecRF<-c(bothF1VecRF, F1_Score(testStd[,12], bothPredictionRF))
...
roseTrain <- ROSE(V12~., trainStd, p=0.5)$data


roseClassDist<-table(roseTrain$V12)
barplot(roseClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(roseTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestRoseSVM<- train(V12~., data = roseTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
rosePrediction <- predict(bestRoseSVM, testStd[1:11])
rosePrediction<- relevel(rosePrediction, "0")
summary(rosePrediction)

#Create a confusion matrix for the best model.
roseTab <- table(pred=rosePrediction, true=testStd[,12])
...
roseF1Vec<-c(roseF1Vec, F1_Score(testStd[,12], rosePrediction))
...
bestRoseRF <-train(V12~., data = roseTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
rosePredictionRF <- predict(bestRoseRF, testStd[1:11])
rosePredictionRF<- relevel(rosePredictionRF, "0")
summary(rosePredictionRF)

#Create a confusion matrix for the best model.
roseTabRF <- table(pred=rosePredictionRF, true=testStd[,12])
...
roseF1VecRF<-c(roseF1VecRF, F1_Score(testStd[,12], rosePredictionRF))
...
smoteTrain<-SMOTE(trainStd[,-12], trainStd[,12], K = 3, dup_size = 0)$data
colnames(smoteTrain)[12] <- "V12"
smoteTrain$V12 <- as.factor(smoteTrain$V12)

smoteClassDist<-table(smoteTrain$V12)
barplot(smoteClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(smoteTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSmoteSVM <- train(V12~., data =smoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
bestSmoteSVM
plot(bestSmoteSVM)
...
#Test the best model on testing data
smotePrediction <- predict(bestSmoteSVM, testStd[1:11])
smotePrediction<- relevel(smotePrediction, "0")
summary(smotePrediction)

#Create a confusion matrix for the best model.
smoteTab <- table(pred=smotePrediction, true=testStd[,12])
...
smoteF1Vec<-c(smoteF1Vec, F1_Score(testStd[,12], smotePrediction))
...
bestSmoteRF <-train(V12~., data = smoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
smotePredictionRF <- predict(bestSmoteRF, testStd[1:11])
smotePredictionRF<- relevel(smotePredictionRF, "0")
summary(smotePredictionRF)

#Create a confusion matrix for the best model.
smoteTabRF <- table(pred=smotePredictionRF, true=testStd[,12])
...
smoteF1VecRF<-c(smoteF1VecRF, F1_Score(testStd[,12], smotePredictionRF))
...
blsTrain<-BLSMOTE(trainStd[,-12], trainStd[,12],K=3,C=3,dupSize=0,method = "type1")$data
colnames(blsTrain)[12] <- "V12"
blsTrain$V12 <- as.factor(blsTrain$V12)

blsClassDist<-table(blsTrain$V12)
barplot(blsClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(blsTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBlsSVM <- train(V12~., data = blsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
blsPrediction <- predict(bestBlsSVM, testStd[1:11])
blsPrediction<- relevel(blsPrediction, "0")
summary(blsPrediction)

#Create a confusion matrix for the best model.
blsTab <- table(pred=blsPrediction, true=testStd[,12])
...
blsF1Vec<-c(blsF1Vec, F1_Score(testStd[,12], blsPrediction))
...
bestBlsRF <-train(V12~., data = blsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
blsPredictionRF <- predict(bestBlsRF, testStd[1:11])
blsPredictionRF<- relevel(blsPredictionRF, "0")
summary(blsPredictionRF)

#Create a confusion matrix for the best model.
blsTabRF <- table(pred=blsPredictionRF, true=testStd[,12])
...
blsF1VecRF<-c(blsF1VecRF, F1_Score(testStd[,12], blsPredictionRF))
...
slsTrain<-SLS(trainStd[,-12], trainStd[,12], K = 3, dupSize = 0)$data
colnames(slsTrain)[12] <- "V12"
slsTrain$V12 <- as.factor(slsTrain$V12)

slsClassDist<-table(slsTrain$V12)
barplot(slsClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(slsTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSlsSVM<- train(V12~., data = slsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestSlsSVM
plot(bestSlsSVM)
...
#Test the best model on testing data
slsPrediction <- predict(bestSlsSVM, testStd[1:11])
slsPrediction<- relevel(slsPrediction, "0")
summary(slsPrediction)

#Create a confusion matrix for the best model.
slsTab <- table(pred=slsPrediction, true=testStd[,12])
...
slsF1Vec<-c(slsF1Vec, F1_Score(testStd[,12], slsPrediction))
...
bestSlsRF <-train(V12~., data = slsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
slsPredictionRF <- predict(bestSlsRF, testStd[1:11])
slsPredictionRF<- relevel(slsPredictionRF, "0")
summary(slsPredictionRF)

#Create a confusion matrix for the best model.
slsTabRF <- table(pred=slsPredictionRF, true=testStd[,12])
...
slsF1VecRF<-c(slsF1VecRF, F1_Score(testStd[,12], slsPredictionRF))
...
adasynTrain<-ADAS(trainStd[,-12], trainStd[,12], K = 3)$data
colnames(adasynTrain)[12] <- "V12"
adasynTrain$V12 <- as.factor(adasynTrain$V12)

adasynClassDist<-table(adasynTrain$V12)
barplot(adasynClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(adasynTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestAdasynSVM <- train(V12~., data = adasynTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                       trControl=trctrl,
                       tuneGrid = tuneValues)
bestAdasynSVM
plot(bestAdasynSVM)
...
#Test the best model on testing data
adasynPrediction <- predict(bestAdasynSVM, testStd[1:11])
adasynPrediction<- relevel(adasynPrediction, "0")
summary(adasynPrediction)

#Create a confusion matrix for the best model.
adasynTab <- table(pred=adasynPrediction, true=testStd[,12])
...
adasynF1Vec<-c(adasynF1Vec, F1_Score(testStd[,12], adasynPrediction))
...
bestAdasynRF <-train(V12~., data = adasynTrain, method = "rf", metric="Kappa",scale = FALSE,
                     ntree=500,
                     trControl=trctrl,
                     tuneGrid = tuneRfValues)
...
adasynPredictionRF <- predict(bestAdasynRF, testStd[1:11])
adasynPredictionRF<- relevel(adasynPredictionRF, "0")
summary(adasynPredictionRF)

#Create a confusion matrix for the best model.
adasynTabRF <- table(pred=adasynPredictionRF, true=testStd[,12])
...
adasynF1VecRF<-c(adasynF1VecRF, F1_Score(testStd[,12], adasynPredictionRF))
...
dbsTrain<-DBSMOTE(trainStd[,-12], trainStd[,12])$data
colnames(dbsTrain)[12] <- "V12"
dbsTrain$V12 <- as.factor(dbsTrain$V12)

dbsClassDist<-table(dbsTrain$V12)
barplot(dbsClassDist, main="Class Distribution", 
        xlab="V12")

ggplot(dbsTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestDbsSVM<- train(V12~., data = dbsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestDbsSVM
plot(bestDbsSVM)
...
#Test the best model on testing data
dbsPrediction <- predict(bestDbsSVM, testStd[1:11])
summary(dbsPrediction)

#Create a confusion matrix for the best model.
dbsTab <- table(pred=dbsPrediction, true=testStd[,12])
...
dbsF1Vec<-c(dbsF1Vec, F1_Score(testStd[,12], dbsPrediction))
...
bestDbsRF <-train(V12~., data = dbsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
dbsPredictionRF <- predict(bestDbsRF, testStd[1:11])
dbsPrediction<- relevel(dbsPrediction, "0")
summary(dbsPredictionRF)

#Create a confusion matrix for the best model.
dbsTabRF <- table(pred=dbsPredictionRF, true=testStd[,12])
...
dbsF1VecRF<-c(dbsF1VecRF, F1_Score(testStd[,12], dbsPredictionRF))
...
#Create a new training set using SMOTE method
tlsmoteTrain<-SMOTE(trainStd[,-12], trainStd[,12], K = 3, dup_size = 0)$data
colnames(tlsmoteTrain)[12] <- "V12"
tlsmoteTrain$V12 <- as.factor(smoteTrain$V12)
#Apply Tomek Links removal
TLsmoteTrain <- TomekClassif(V12~., tlsmoteTrain, dist = "Euclidean", 
                             Cl = "all", rem = "both")[[1]]

tlsmoteClassDist<-table(TLsmoteTrain$V12)
barplot(tlsmoteClassDist, main="Class Distribution", 
        xlab="Survival Status")

ggplot(TLsmoteTrain, aes(x=V1, y=V3, color=V12)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
besttlSmoteSVM <- train(V12~., data =TLsmoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                        trControl=trctrl,
                        tuneGrid = tuneValues)
besttlSmoteSVM
plot(besttlSmoteSVM)
...
#Test the best model on testing data
tlsmotePrediction <- predict(besttlSmoteSVM, testStd[1:11])
summary(tlsmotePrediction)

#Create a confusion matrix for the best model.
tlsmoteTab <- table(pred=tlsmotePrediction, true=testStd[,12])
...
tlsmoteF1Vec<-c(tlsmoteF1Vec, F1_Score(testStd[,12], tlsmotePrediction))
...
besttlSmoteRF <-train(V12~., data = TLsmoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                      ntree=500,
                      trControl=trctrl,
                      tuneGrid = tuneRfValues)
...
tlsmotePredictionRF <- predict(besttlSmoteRF, testStd[1:11])
summary(tlsmotePredictionRF)

#Create a confusion matrix for the best model.
tlsmoteTabRF <- table(pred=tlsmotePredictionRF, true=testStd[,12])
...
tlsmoteF1VecRF<-c(tlsmoteF1VecRF, F1_Score(testStd[,12], tlsmotePredictionRF))
...
save.image(file='WhiteWineMyEnvironment.RData')
dir()
load('WhiteWineMyEnvironment.RData')
