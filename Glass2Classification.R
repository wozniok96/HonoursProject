...
df <- read.csv("glass2.dat", header= FALSE, stringsAsFactors = FALSE,
               skip = 14)
colnames(df)

#Change class value into integers
df$V10[df$V10 == " positive"] <- 0
df$V10[df$V10 == " negative"] <- 1
df$V10<- as.factor(df$V10)

classDist<-table(df$V10)
...
train.index <- createDataPartition(df$V10, p = .7, list = FALSE)
...
ggplot(trainStd, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)
...
bestSVM<- train(V10~., data = trainStd, method = "svmRadial", metric="Kappa", scale = FALSE,
                trControl=trctrl,
                tuneGrid = tuneValues)
...
bestRF <-train(V10~., data = trainStd, method = "rf", metric="Kappa",scale = FALSE,
               ntree=500,
               trControl=trctrl,
...
PredictionRF <- predict(bestRF, testStd[1:9])
summary(PredictionRF)

#Create a confusion matrix for the best model.
TabRF <- table(pred=PredictionRF, true=testStd[,10])
ResultsRF <- confusionMatrix(TabRF)
ResultsRF

sensVecRF<-c(sensVecRF, ResultsRF$byClass[[1]])
specVecRF<-c(specVecRF, ResultsRF$byClass[[2]])
F1VecRF<-c(F1VecRF, F1_Score(testStd[,10], PredictionRF))
F1VecRF<-ifelse(is.nan(F1VecRF),0,F1VecRF)
...
weighedTuneValues <- expand.grid(sigma = c(0.01, 0.05, 0.1, 0.5, 0.9),
                                 ...
                                 Weight=c(3,5,8,10,15))

weighedBestSVM<- train(V10~., data = trainStd, method = "svmRadialWeights",  metric="Kappa",scale = FALSE,
                       trControl=trctrl,
...
#Test the best model on testing data
weighedPrediction <- predict(weighedBestSVM, testStd[1:9])
summary(weighedPrediction)
weighedTab <- table(pred=weighedPrediction, true=testStd[,10])
...
weighedF1Vec<-c(weighedF1Vec, F1_Score(testStd[,10], weighedPrediction))
...
overTrain <- ovun.sample(V10~., trainStd, method="over", p=.5)$data


overClassDist<-table(overTrain$V10)
barplot(overClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(overTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestOverSVM<- train(V10~., data = overTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
overPrediction <- predict(bestOverSVM, testStd[1:9])
overPrediction<- relevel(overPrediction, "0")
#Create a confusion matrix for the best model.
overTab <- table(pred=overPrediction, true=testStd[,10])
...
overF1Vec<-c(overF1Vec, F1_Score(testStd[,10], overPrediction))
...
bestOverRF <-train(V10~., data = overTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
overPredictionRF <- predict(bestOverRF, testStd[1:9])
overPredictionRF<- relevel(overPredictionRF, "0")
summary(overPredictionRF)

#Create a confusion matrix for the best model.
overTabRF <- table(pred=overPredictionRF, true=testStd[,10])
...
overF1VecRF<-c(overF1VecRF, F1_Score(testStd[,10], overPredictionRF))
overF1VecRF<-ifelse(is.nan(overF1VecRF),0,overF1VecRF)
...
underTrain <- ovun.sample(V10~., trainStd, method="under", p=.5)$data


underClassDist<-table(underTrain$V10)
barplot(underClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(underTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestUnderSVM <- train(V10~., data = underTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
...
#Test the best model on testing data
underPrediction <- predict(bestUnderSVM, testStd[1:9])
underPrediction<- relevel(underPrediction, "0")
summary(underPrediction)

#Create a confusion matrix for the best model.
underTab <- table(pred=underPrediction, true=testStd[,10])
...
underF1Vec<-c(underF1Vec, F1_Score(testStd[,10], underPrediction))
...
bestUnderRF <-train(V10~., data = underTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
underPredictionRF <- predict(bestUnderRF, testStd[1:9])
underPredictionRF<- relevel(underPredictionRF, "0")
summary(underPredictionRF)

#Create a confusion matrix for the best model.
underTabRF <- table(pred=underPredictionRF, true=testStd[,10])
...
underF1VecRF<-c(underF1VecRF, F1_Score(testStd[,10], underPredictionRF))
...
#Create new training set using undersampling.
bothTrain <- ovun.sample(V10~., trainStd, method="both", p=.5)$data


bothClassDist<-table(bothTrain$V10)
barplot(bothClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(bothTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBothSVM <-train(V10~., data = bothTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
bothPrediction <- predict(bestBothSVM, testStd[1:9])
bothPrediction<- relevel(bothPrediction, "0")
summary(bothPrediction)

#Create a confusion matrix for the best model.
bothTab <- table(pred=bothPrediction, true=testStd[,10])
...
bothF1Vec<-c(bothF1Vec, F1_Score(testStd[,10], bothPrediction))
...
bestBothRF <-train(V10~., data = bothTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
bothPredictionRF <- predict(bestBothRF, testStd[1:9])
bothPredictionRF<- relevel(bothPredictionRF, "0")
summary(bothPredictionRF)

#Create a confusion matrix for the best model.
bothTabRF <- table(pred=bothPredictionRF, true=testStd[,10])
...
bothF1VecRF<-c(bothF1VecRF, F1_Score(testStd[,10], bothPredictionRF))
...
roseTrain <- ROSE(V10~., trainStd, p=0.5)$data


roseClassDist<-table(roseTrain$V10)
barplot(roseClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(roseTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestRoseSVM<- train(V10~., data = roseTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
rosePrediction <- predict(bestRoseSVM, testStd[1:9])
rosePrediction<- relevel(rosePrediction, "0")
summary(rosePrediction)

#Create a confusion matrix for the best model.
roseTab <- table(pred=rosePrediction, true=testStd[,10])
...
roseF1Vec<-c(roseF1Vec, F1_Score(testStd[,10], rosePrediction))
...
bestRoseRF <-train(V10~., data = roseTrain, method = "rf", metric="Kappa",scale = FALSE,
                   ntree=500,
                   trControl=trctrl,
                   tuneGrid = tuneRfValues)
...
rosePredictionRF <- predict(bestRoseRF, testStd[1:9])
rosePredictionRF<- relevel(rosePredictionRF, "0")
summary(rosePredictionRF)

#Create a confusion matrix for the best model.
roseTabRF <- table(pred=rosePredictionRF, true=testStd[,10])
...
roseF1VecRF<-c(roseF1VecRF, F1_Score(testStd[,10], rosePredictionRF))
...
smoteTrain<-SMOTE(trainStd[,-10], trainStd[,10], K = 3, dup_size = 0)$data
colnames(smoteTrain)[10] <- "V10"
smoteTrain$V10 <- as.factor(smoteTrain$V10)

smoteClassDist<-table(smoteTrain$V10)
barplot(smoteClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(smoteTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSmoteSVM <- train(V10~., data =smoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                      trControl=trctrl,
                      tuneGrid = tuneValues)
bestSmoteSVM
plot(bestSmoteSVM)
...
#Test the best model on testing data
smotePrediction <- predict(bestSmoteSVM, testStd[1:9])
smotePrediction<- relevel(smotePrediction, "0")
summary(smotePrediction)

#Create a confusion matrix for the best model.
smoteTab <- table(pred=smotePrediction, true=testStd[,10])
...
smoteF1Vec<-c(smoteF1Vec, F1_Score(testStd[,10], smotePrediction))
...
bestSmoteRF <-train(V10~., data = smoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                    ntree=500,
                    trControl=trctrl,
                    tuneGrid = tuneRfValues)
...
smotePredictionRF <- predict(bestSmoteRF, testStd[1:9])
smotePredictionRF<- relevel(smotePredictionRF, "0")
summary(smotePredictionRF)

#Create a confusion matrix for the best model.
smoteTabRF <- table(pred=smotePredictionRF, true=testStd[,10])
...
smoteF1VecRF<-c(smoteF1VecRF, F1_Score(testStd[,10], smotePredictionRF))
smoteF1VecRF<-ifelse(is.nan(smoteF1VecRF),0,smoteF1VecRF)
...
blsTrain<-BLSMOTE(trainStd[,-10], trainStd[,10],K=3,C=3,dupSize=0,method = "type1")$data
colnames(blsTrain)[10] <- "V10"
blsTrain$V10 <- as.factor(blsTrain$V10)

blsClassDist<-table(blsTrain$V10)
barplot(blsClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(blsTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestBlsSVM <- train(V10~., data = blsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                    trControl=trctrl,
                    tuneGrid = tuneValues)
...
#Test the best model on testing data
blsPrediction <- predict(bestBlsSVM, testStd[1:9])
blsPrediction<- relevel(blsPrediction, "0")
summary(blsPrediction)

#Create a confusion matrix for the best model.
blsTab <- table(pred=blsPrediction, true=testStd[,10])
...
blsF1Vec<-c(blsF1Vec, F1_Score(testStd[,10], blsPrediction))
...
bestBlsRF <-train(V10~., data = blsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
blsPredictionRF <- predict(bestBlsRF, testStd[1:9])
blsPredictionRF<- relevel(blsPredictionRF, "0")
summary(blsPredictionRF)

#Create a confusion matrix for the best model.
blsTabRF <- table(pred=blsPredictionRF, true=testStd[,10])
...
blsF1VecRF<-c(blsF1VecRF, F1_Score(testStd[,10], blsPredictionRF))
blsF1VecRF<-ifelse(is.nan(blsF1VecRF),0,blsF1VecRF)
...
slsTrain<-SLS(trainStd[,-10], trainStd[,10], K = 3, dupSize = 0)$data
colnames(slsTrain)[10] <- "V10"
slsTrain$V10 <- as.factor(slsTrain$V10)

slsClassDist<-table(slsTrain$V10)
barplot(slsClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(slsTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestSlsSVM<- train(V10~., data = slsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestSlsSVM
plot(bestSlsSVM)
...
#Test the best model on testing data
slsPrediction <- predict(bestSlsSVM, testStd[1:9])
slsPrediction<- relevel(slsPrediction, "0")
summary(slsPrediction)

#Create a confusion matrix for the best model.
slsTab <- table(pred=slsPrediction, true=testStd[,10])
...
slsF1Vec<-c(slsF1Vec, F1_Score(testStd[,10], slsPrediction))
...
bestSlsRF <-train(V10~., data = slsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
slsPredictionRF <- predict(bestSlsRF, testStd[1:9])
slsPredictionRF<- relevel(slsPredictionRF, "0")
summary(slsPredictionRF)

#Create a confusion matrix for the best model.
slsTabRF <- table(pred=slsPredictionRF, true=testStd[,10])
...
slsF1VecRF<-c(slsF1VecRF, F1_Score(testStd[,10], slsPredictionRF))
slsF1VecRF<-ifelse(is.nan(slsF1VecRF),0,slsF1VecRF)
...
adasynTrain<-ADAS(trainStd[,-10], trainStd[,10], K = 3)$data
colnames(adasynTrain)[10] <- "V10"
adasynTrain$V10 <- as.factor(adasynTrain$V10)

adasynClassDist<-table(adasynTrain$V10)
barplot(adasynClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(adasynTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestAdasynSVM <- train(V10~., data = adasynTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                       trControl=trctrl,
                       tuneGrid = tuneValues)
bestAdasynSVM
plot(bestAdasynSVM)
...
#Test the best model on testing data
adasynPrediction <- predict(bestAdasynSVM, testStd[1:9])
adasynPrediction<- relevel(adasynPrediction, "0")
summary(adasynPrediction)

#Create a confusion matrix for the best model.
adasynTab <- table(pred=adasynPrediction, true=testStd[,10])
...
adasynF1Vec<-c(adasynF1Vec, F1_Score(testStd[,10], adasynPrediction))
...
bestAdasynRF <-train(V10~., data = adasynTrain, method = "rf", metric="Kappa",scale = FALSE,
                     ntree=500,
                     trControl=trctrl,
                     tuneGrid = tuneRfValues)
...
adasynPredictionRF <- predict(bestAdasynRF, testStd[1:9])
adasynPredictionRF<- relevel(adasynPredictionRF, "0")
summary(adasynPredictionRF)

#Create a confusion matrix for the best model.
adasynTabRF <- table(pred=adasynPredictionRF, true=testStd[,10])
...
adasynF1VecRF<-c(adasynF1VecRF, F1_Score(testStd[,10], adasynPredictionRF))
adasynF1VecRF<-ifelse(is.nan(adasynF1VecRF),0,adasynF1VecRF)
...
dbsTrain<-DBSMOTE(trainStd[,-10], trainStd[,10] )$data
colnames(dbsTrain)[10] <- "V10"
dbsTrain$V10 <- as.factor(dbsTrain$V10)

dbsClassDist<-table(dbsTrain$V10)
barplot(dbsClassDist, main="Class Distribution", 
        xlab="V10")

ggplot(dbsTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
bestDbsSVM<- train(V10~., data = dbsTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                   trControl=trctrl,
                   tuneGrid = tuneValues)
bestDbsSVM
plot(bestDbsSVM)
...
#Test the best model on testing data
dbsPrediction <- predict(bestDbsSVM, testStd[1:9])
summary(dbsPrediction)

#Create a confusion matrix for the best model.
dbsTab <- table(pred=dbsPrediction, true=testStd[,10])
...
dbsF1Vec<-c(dbsF1Vec, F1_Score(testStd[,10], dbsPrediction))
...
bestDbsRF <-train(V10~., data = dbsTrain, method = "rf", metric="Kappa",scale = FALSE,
                  ntree=500,
                  trControl=trctrl,
                  tuneGrid = tuneRfValues)
...
dbsPredictionRF <- predict(bestDbsRF, testStd[1:9])
dbsPrediction<- relevel(dbsPrediction, "0")
summary(dbsPredictionRF)

#Create a confusion matrix for the best model.
dbsTabRF <- table(pred=dbsPredictionRF, true=testStd[,10])
...
dbsF1VecRF<-c(dbsF1VecRF, F1_Score(testStd[,10], dbsPredictionRF))
dbsF1VecRF<-ifelse(is.nan(dbsF1VecRF),0,dbsF1VecRF)
...
#Create a new training set using SMOTE method
tlsmoteTrain<-SMOTE(trainStd[,-10], trainStd[,10], K = 3, dup_size = 0)$data
colnames(tlsmoteTrain)[10] <- "V10"
tlsmoteTrain$V10 <- as.factor(smoteTrain$V10)
#Apply Tomek Links removal
TLsmoteTrain <- TomekClassif(V10~., tlsmoteTrain, dist = "Euclidean", 
                             Cl = "all", rem = "both")[[1]]

tlsmoteClassDist<-table(TLsmoteTrain$V10)
barplot(tlsmoteClassDist, main="Class Distribution", 
        xlab="Survival Status")

ggplot(TLsmoteTrain, aes(x=V4, y=V5, color=V10)) + geom_point(shape=16)


#Find the best model using range of parameters (gridsearch)
besttlSmoteSVM <- train(V10~., data =TLsmoteTrain, method = "svmRadial", metric="Kappa",scale = FALSE,
                        trControl=trctrl,
                        tuneGrid = tuneValues)
besttlSmoteSVM
plot(besttlSmoteSVM)
...
#Test the best model on testing data
tlsmotePrediction <- predict(besttlSmoteSVM, testStd[1:9])
summary(tlsmotePrediction)

#Create a confusion matrix for the best model.
tlsmoteTab <- table(pred=tlsmotePrediction, true=testStd[,10])
...
tlsmoteF1Vec<-c(tlsmoteF1Vec, F1_Score(testStd[,10], tlsmotePrediction))
...
besttlSmoteRF <-train(V10~., data = TLsmoteTrain, method = "rf", metric="Kappa",scale = FALSE,
                      ntree=500,
                      trControl=trctrl,
                      tuneGrid = tuneRfValues)
...
tlsmotePredictionRF <- predict(besttlSmoteRF, testStd[1:9])
summary(tlsmotePredictionRF)

#Create a confusion matrix for the best model.
tlsmoteTabRF <- table(pred=tlsmotePredictionRF, true=testStd[,10])
...
tlsmoteF1VecRF<-c(tlsmoteF1VecRF, F1_Score(testStd[,10], tlsmotePredictionRF))
...
save.image(file='Glass2MyEnvironment.RData')
dir()
load('Glass2MyEnvironment.RData')
